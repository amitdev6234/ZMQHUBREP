...
int main(int argc, char *argv[]) {
	...
	while (1)
	{
		...
		std::chrono::microseconds microseconds_since_epoch = (std::chrono::duration_cast<std::chrono::microseconds>(tp));
		
//		std::string txt = String::Format("The value is {0}", microseconds_since_epoch.count());
		fillResponse[strlen(fillResponse) - 1] = '\0';

		std::cout << microseconds_since_epoch.count() << " Received [" << fillResponse << "]" << std::endl;
		
		s_send(publisher, fillResponse);
		free(fillResponse);
		//s_send(nsefeedrcpt, "0");
	}
	zmq_close(publisher);
